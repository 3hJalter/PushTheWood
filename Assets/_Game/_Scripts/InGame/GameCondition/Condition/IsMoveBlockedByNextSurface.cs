...
{
    [CreateAssetMenu(fileName = "IsMoveBlockedByNextSurface", menuName = "ConditionSO/Moving/IsMoveBlockedByNextSurface", order = 1)]
    public class IsMoveBlockedByNextSurface : ScriptableObject, ICondition
    {
        public bool IsApplicable(ConditionData data)
        {
            ...
            for (int i = 0; i < movingData.owner.cellInUnits.Count; i++)
            {
                ...
                if (neighbourHeight > minOwnerHeight)
                {
                    minOwnerHeight = neighbourHeight;
                }
            }
