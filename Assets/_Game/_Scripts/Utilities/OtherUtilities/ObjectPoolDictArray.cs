...
namespace _Game.daivq.Utilities
{
    ...
    {
        private readonly Dictionary<int, List<GameObject>> _pooledGoDic = new();
        private readonly List<int> _pooledKeyList = new();
        ...
        public int GetActivePooledObjectCount()
        {
            ...
            for (int i = 0; i < _pooledKeyList.Count; i++)
            {
                ...
                for (int j = 0; j < goList.Count; j++)
                {
                    ...
                    if (go != null && go.activeInHierarchy) cnt++;
                }
            }
        ...
        public List<GameObject> GetActiveObjects(GameObject prefab)
        {
            ...
            if (_pooledKeyList.Contains(key) == false && _pooledGoDic.ContainsKey(key) == false) return null;

            List<GameObject> listActiveObjecs = new();
            ...
            for (int i = goList.Count - 1; i >= 0; i--)
            {
                ...
                if (go.activeSelf) listActiveObjecs.Add(go);
            }
        ...
        private readonly Dictionary<int, Coroutine> _dictCorPrePooling = new();

        /// <summary>
        ///     Set amountSpawnEachFrame=-1 for instant pre-pooling
        /// </summary>
        public void PrePooling(GameObject prefab, int amount, int amountSpawnEachFrame, Action onEnd = null)
        {
            ...
            if (amountSpawnEachFrame > 0)
            {
                ...
                {
                    _dictCorPrePooling.Add(key,
                        StartCoroutine(IEPrePooling(prefab, amount, amountSpawnEachFrame, onEnd)));
                }
            }
            else
            {
        ...
        public void PrePoolingMax(GameObject prefab, int maxAmount, int amountSpawnEachFrame, Action onEnd = null)
        {
            int currentAmount = CountSpawnedObjects(prefab);
            PrePooling(prefab, maxAmount - currentAmount, amountSpawnEachFrame, onEnd);
        }

        private IEnumerator IEPrePooling(GameObject prefab, int amount, int amountSpawnEachFrame, Action onEnd = null)
        {
        ...
        private void InstantPrePooling(GameObject prefab, int amount)
        {
            ...
            for (int i = 0; i < amount; i++)
            {
                go = Instantiate(prefab, _Transform);
                go.SetActive(false);
                goList.Add(go);
            }
        }
            ...
            bool forceInstantiate = false)
        {
            ...
            if (!EditorApplication.isPlaying)
            {
                GameObject objectEditor = (GameObject)PrefabUtility.InstantiatePrefab(prefab);
        ...
        public T GetGameObject<T>(T prefabComponent, Vector3 position, Quaternion rotation,
            bool forceInstantiate = false) where T : MonoBehaviour
        {
        ...
        public ParticleSystem GetParticle(ParticleSystem particle, Vector3 position, Quaternion rotation,
            bool forceInstantiate = false)
        {
        ...
        public void PlayParticleOnce(ParticleSystem particle, Vector3 position, float scale = -1)
        {
            ...
            GameObject go = GetGameObject(particle.gameObject, position, particle.transform.rotation);
            if (scale > 0f) go.transform.localScale = particle.transform.localScale * scale;
        ...
        public LineRenderer GetLine(LineRenderer line, Vector3 position, Quaternion rotation,
            bool forceInstantiate = false)
        {
        ...
        public List<GameObject> GetGameObjects(GameObject prefab, int amount, bool forceInstantiate = false)
        {
            ...
            if (!EditorApplication.isPlaying)
            {
                List<GameObject> objectsEditor = new();
                for (int i = 0; i < amount; i++) objectsEditor.Add((GameObject)PrefabUtility.InstantiatePrefab(prefab));
            ...
            List<GameObject> goList = _pooledGoDic[key];
            List<GameObject> listReadyGameObject = new();
        ...
        public List<GameObject> GetGameObjects(GameObject prefab, int amount, Vector3 position, Quaternion rotation,
            bool forceInstantiate = false)
        {
            if (prefab == null) return null;
        ...
        public List<T> GetGameObjects<T>(T prefabComponent, int amount, bool forceInstantiate = false)
            where T : MonoBehaviour
        {
            if (prefabComponent == null) return null;
            List<GameObject> gos = GetGameObjects(prefabComponent.gameObject, amount, forceInstantiate);
            List<T> components = new();
            for (int i = 0; i < gos.Count; i++) components.Add(gos[i].GetComponent<T>());
            return components;
        }
        ...
        public List<ParticleSystem> GetParticles(ParticleSystem particle, int amount, bool forceInstantiate = false)
        {
            ...
            List<ParticleSystem> components = new();
            for (int i = 0; i < gos.Count; i++) components.Add(gos[i].GetComponent<ParticleSystem>());
            return components;
        }
        ...
        public List<LineRenderer> GetLines(LineRenderer line, int amount, bool forceInstantiate = false)
        {
            ...
            List<LineRenderer> components = new();
            for (int i = 0; i < gos.Count; i++) components.Add(gos[i].GetComponent<LineRenderer>());
            return components;
        }

        public List<T> GetGameObjects<T>(T prefabComponent, int amount, Vector3 position, Quaternion rotation,
            bool forceInstantiate = false) where T : MonoBehaviour
        {
            if (prefabComponent == null) return null;
            List<GameObject> gos =
                GetGameObjects(prefabComponent.gameObject, amount, position, rotation, forceInstantiate);
            List<T> components = new();
            for (int i = 0; i < gos.Count; i++) components.Add(gos[i].GetComponent<T>());
            return components;
        }

        public List<ParticleSystem> GetParticles(ParticleSystem particle, int amount, Vector3 position,
            Quaternion rotation, bool forceInstantiate = false)
        {
            if (particle == null) return null;
        ...
        public List<LineRenderer> GetLines(LineRenderer line, int amount, Vector3 position, Quaternion rotation,
            bool forceInstantiate = false)
        {
            if (line == null) return null;
        ...
        public Coroutine GetGameObjectsIterate(GameObject prefab, int amount, int amountEachFrame,
            Action<GameObject> onGetObject, Action onFinished = null)
        {
            ...
            if (countReadyObjects < amount)
                return StartCoroutine(IESpawnGameObjectIterate(goList, prefab, amount - countReadyObjects,
                    amountEachFrame, onGetObject, onFinished));

            onFinished?.Invoke();
            return null;

            IEnumerator IESpawnGameObjectIterate(List<GameObject> goList, GameObject prefab, int amount,
                int amountEachFrame, Action<GameObject> onGetObject, Action onFinished)
            {
                // extend list capacity
        ...
        public Coroutine GetGameObjectsIterate(GameObject prefab, int amount, Vector3 position, Quaternion rotation,
            int amountEachFrame, Action<GameObject> onGetObject, Action onFinished = null)
        {
            if (prefab == null || onGetObject == null)
            {
                onFinished?.Invoke();
            ...
            if (countReadyObjects < amount)
                return StartCoroutine(IESpawnGameObjectIterate(goList, prefab, amount - countReadyObjects, position,
                    rotation, amountEachFrame, onGetObject, onFinished));

            onFinished?.Invoke();
            return null;

            IEnumerator IESpawnGameObjectIterate(List<GameObject> goList, GameObject prefab, int amount,
                Vector3 position, Quaternion rotation, int amountEachFrame, Action<GameObject> onGetObject,
                Action onFinished)
            {
                // extend list capacity
                goList.Capacity = Mathf.Max(goList.Capacity, goList.Count + amount);
        ...
        public Coroutine GetGameObjectsIterate<T>(T prefabComponent, int amount, int amountEachFrame,
            Action<T> onGetComponent, Action onFinished = null) where T : MonoBehaviour
        {
            if (prefabComponent == null) return null;
            return GetGameObjectsIterate(prefabComponent.gameObject, amount, amountEachFrame, OnGetGameObject,
                onFinished);
        ...
        public Coroutine GetParticles(ParticleSystem particle, int amount, int amountEachFrame,
            Action<ParticleSystem> onGetParticle, Action onFinished = null)
        {
            if (particle == null) return null;
        ...
        public Coroutine GetLines(LineRenderer line, int amount, int amountEachFrame, Action<LineRenderer> onGetLine,
        ...
        public Coroutine GetGameObjectsIterate<T>(T prefabComponent, int amount, Vector3 position, Quaternion rotation,
            int amountEachFrame, Action<T> onGetComponent, Action onFinished = null) where T : MonoBehaviour
        {
            if (prefabComponent == null) return null;
            return GetGameObjectsIterate(prefabComponent.gameObject, amount, position, rotation, amountEachFrame,
                OnGetGameObject, onFinished);
        ...
        public Coroutine GetParticles(ParticleSystem particle, int amount, Vector3 position, Quaternion rotation,
            int amountEachFrame, Action<ParticleSystem> onGetParticle, Action onFinished = null)
        {
            if (particle == null) return null;
            return GetGameObjectsIterate(particle.gameObject, amount, position, rotation, amountEachFrame,
                OnGetGameObject, onFinished);
        ...
        public Coroutine GetLines(LineRenderer line, int amount, Vector3 position, Quaternion rotation,
            int amountEachFrame, Action<LineRenderer> onGetLine, Action onFinished = null)
        {
            if (line == null) return null;
            return GetGameObjectsIterate(line.gameObject, amount, position, rotation, amountEachFrame, OnGetGameObject,
                onFinished);
