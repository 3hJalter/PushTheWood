...
namespace VinhLB
{
    public class BuoyantObject : HMonoBehaviour
    {
        ...
        [Header("Water")] [SerializeField] private float _waterHeight;

        [Header("Waves")] [SerializeField] private float _steepness;

        [SerializeField] private float _wavelength;

        [SerializeField] private float _speed;

        [SerializeField] private float[] _directionArray = new float[4];

        [Header("Buoyancy")] [SerializeField] [Range(1.0f, 5.0f)]
        private float _strength = 1.0f;

        [SerializeField] [Range(0.2f, 5.0f)] private float _objectDepth = 1.0f;

        [SerializeField] private float _velocityDrag = 0.99f;

        [SerializeField] private float _angularDrag = 0.5f;

        [Header("Effectors")] [SerializeField] private Transform[] _effectorArray;
        ...
        private void OnDrawGizmos()
        {
            ...
            for (int i = 0; i < _effectorArray.Length; i++)
                ...
                else
                {
                    ...
                    Gizmos.color =
                        _effectorArray[i].position.y < _effectorProjectionArray[i].y ? _red : _green; // submerged
