...
namespace VinhLB
{
    public class BuoyantObject : MonoBehaviour
    {
        ...
        private Vector3[] _effectorProjectionArray;
        ...
        private void Awake()
        {
            ...
            _effectorProjectionArray = new Vector3[_effectorArray.Length];
            for (int i = 0; i < _effectorArray.Length; i++) _effectorProjectionArray[i] = _effectorArray[i].position;
        }
        ...
        private void FixedUpdate()
        {
            ...
            for (int i = 0; i < effectorAmount; i++)
            {
                ...
                _effectorProjectionArray[i] = effectorPosition;
                _effectorProjectionArray[i].y = _waterHeight + 
                    GerstnerWaveDisplacement.GetWaveDisplacement(effectorPosition, _steepness, _wavelength, _speed, _directionArray).y;

                // gravity
                _rigidbody.AddForceAtPosition(Physics.gravity / effectorAmount, effectorPosition, ForceMode.Acceleration);

                float waveHeight = _effectorProjectionArray[i].y;
                ...
                // drag
                _rigidbody.AddForce(-_rigidbody.velocity * (_velocityDrag * Time.fixedDeltaTime), ForceMode.VelocityChange);

                // torque
                _rigidbody.AddTorque(-_rigidbody.angularVelocity * (_angularDrag * Time.fixedDeltaTime), ForceMode.Impulse);
            }
        }
        ...
        private void OnDrawGizmos()
        {
            ...
            for (int i = 0; i < _effectorArray.Length; i++)
            {
                ...
                else
                {
                    ...
                    Gizmos.color = _effectorArray[i].position.y < _effectorProjectionArray[i].y ? _red : _green; // submerged

                    Gizmos.DrawSphere(_effectorArray[i].position, 0.06f);

                    Gizmos.color = _orange;
                    Gizmos.DrawSphere(_effectorProjectionArray[i], 0.06f);

                    Gizmos.color = _blue;
                    Gizmos.DrawLine(_effectorArray[i].position, _effectorProjectionArray[i]);
                }
            }
        }
    }
