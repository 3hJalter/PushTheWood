...
public class BF_TractorCamera : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        Vector3 desiredPosition = target.position + offset +
                                  new Vector3(-target.forward.x * 8f, -target.forward.y, -target.forward.z * 8f);
        Vector3 desiredLook =
            target.position + new Vector3(target.forward.x * 8f, target.forward.y, target.forward.z * 8f);
