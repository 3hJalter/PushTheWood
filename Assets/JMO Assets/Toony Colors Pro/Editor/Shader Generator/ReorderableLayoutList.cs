...
public class ReorderableLayoutList
{
        ...
        float dragWidth = DRAG_WIDTH)
    {
        ...
        Action<int> DrawListItem = i =>
        {
            ...
            //get dragging rect
            Rect dragRect = EditorGUILayout.BeginVertical();
            {
                if (draggedElement == i)
                {
                    float c = EditorGUIUtility.isProSkin ? 0.2f : 0.75f;
                ...
                if (Event.current.type == EventType.Repaint)
                {
                    ...
                    int count = Mathf.FloorToInt(drawRect.height / dragHeight);
                    count = Mathf.Max(1, count);
                    float margin = drawRect.height - count * dragHeight;
                    for (int j = 0; j < count; j++)
                    {
                        Rect dragIconRect = drawRect;
                        dragIconRect.xMin += 5;
                        dragIconRect.xMax -= 5;
        ...
        if (draggedElement > -1 && Event.current.type == EventType.MouseDrag)
        {
            ...
            for (int i = 0; i < elementRects.Length; i++)
                if (elementRects[i].Contains(mousePosition) && draggedElement != i)
                {
                    ...
                    float diff = elementRects[i].y - elementRects[draggedElement].y;
                    mouseDragOrigin.y += diff;

                    //compensate size difference between swapped implementations
                    float heightDiff = lastSwappedHeight - elementRects[i].height;
                    lastSwappedHeight = elementRects[i].height;
