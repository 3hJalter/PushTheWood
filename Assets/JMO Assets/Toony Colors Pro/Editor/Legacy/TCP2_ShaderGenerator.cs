...
namespace ToonyColorsPro
{
    namespace Legacy
    {
        public partial class TCP2_ShaderGenerator : EditorWindow
        {
            ...
            {
                TCP2_ShaderGenerator window = GetWindow<TCP2_ShaderGenerator>(true, "TCP2 : Shader Generator", true);
            ...
            public class ShaderGeneratorTemplate
            {
                ...
                {
                    foreach (UIFeature uiFeature in uiFeatures)
                ...
                private TextAsset LoadTextAsset(string filename)
                {
                    ...
                    TextAsset asset =
                        AssetDatabase.LoadAssetAtPath<TextAsset>(string.Format("{0}/Shader Templates/{1}", rootPath,
                            filename));
                    if (asset == null)
                    {
                        string filenameNoExtension = Path.GetFileNameWithoutExtension(filename);
                        string[] guids =
                            AssetDatabase.FindAssets(string.Format("{0} t:TextAsset", filenameNoExtension));
                        if (guids.Length >= 1)
                        {
            ...
            private TextAsset[] LoadAllTemplates()
            {
                ...
                string systemPath = Application.dataPath + @"/JMO Assets/Toony Colors Pro/Shader Templates/";
            ...
            private static readonly List<string> sOpenedFoldouts = new();
	...
	private ShaderImporter mCurrentShaderImporter;
            ...
            private void OnEnable()
            {
                ...
                TextAsset[] allTemplates = LoadAllTemplates();
            ...
            private void OnGUI()
            {
                ...
                */
                ...
                if (mDirtyConfig)
                {
                    GUIContent guiContent = new GUIContent("Unsaved changes");
                    Rect rect = GUILayoutUtility.GetRect(guiContent, EditorStyles.helpBox, GUILayout.Height(16));
                    rect.y -= 2;
                ...
                {
                    if (GUILayout.Button("Copy", EditorStyles.miniButtonLeft, GUILayout.Width(60f),
                            GUILayout.Height(16))) CopyShader();
                }
                ...
                {
                    EditorGUILayout.HelpBox(
                        "This template is for the Shader Generator 2 only.\nThis is the Shader Generator 1.",
                        MessageType.Error);
                }
 ...
 ShaderImporter.GetAtPath(AssetDatabase.GetAssetPath(mCurrentShader)) as ShaderImporter;
			...
			if (mCurrentShaderImporter != null && mCurrentShaderImporter.GetShader() == mCurrentShader)
			{
                ...
                GUILayout.FlexibleSpace();
                GUI.color = mDirtyConfig ? gColor * unsavedChangesColor : GUI.color;
                if (GUILayout.Button(mCurrentShader == null ? "Generate Shader" : "Update Shader",
                        GUILayout.Width(120f), GUILayout.Height(30f)))
                {
                    if (Template == null)
                    {
                    ...
                    Shader generatedShader = TCP2_ShaderGeneratorUtils.Compile(mCurrentConfig, mCurrentShader, Template,
                        true, !sOverwriteConfigs);
                    ReloadUserShaders();
                ...
                sOverwriteConfigs = GUILayout.Toggle(sOverwriteConfigs,
                    new GUIContent("Always overwrite shaders",
                ...
                TCP2_ShaderGeneratorUtils.CustomOutputDir = GUILayout.Toggle(TCP2_ShaderGeneratorUtils.CustomOutputDir,
                    new GUIContent("Custom Output Directory:",
                        "Will save the generated shaders in a custom directory within the Project"),
                ...
                if (TCP2_ShaderGeneratorUtils.CustomOutputDir)
                {
                    TCP2_ShaderGeneratorUtils.OutputPath =
                        EditorGUILayout.TextField("", TCP2_ShaderGeneratorUtils.OutputPath);
            ...
            private string[] GetShaderLabels(Shader[] array, string firstOption = "New Shader")
            {
                ...
                foreach (Shader shader in array) labelsList.Add(shader.name);
