    ...
    namespace Legacy
    {
        public static class TCP2_ShaderGeneratorUtils
        {
            ...
            public static string OutputPath
            {
                get
                {
                    if (!CustomOutputDir)
                    {
                        ...
                        {
                            string rootPath = Utils.FindReadmePath(true).Substring("Assets".Length);
                            if (!string.IsNullOrEmpty(rootPath)) return rootPath + "/Shaders Generated/";
                        }
                ...
                set
                {
                    if (_OutputPath != value)
                    {
                        ...
                        foreach (char c in Path.GetInvalidPathChars()) value = value.Replace(c.ToString(), "");
            ...
            public static Shader Compile(TCP2_Config config, Shader existingShader,
                TCP2_ShaderGenerator.ShaderGeneratorTemplate template, bool showProgressBar = true,
                ...
                TCP2_ShaderGenerator.ShaderGeneratorTemplate template, float progress, bool overwritePrompt)
            {
                ...
                //Save to disk
                Shader shader = SaveShader(config, existingShader, source, overwritePrompt,
                ...
                if (config.configType == "terrain")
                {
                    ...
                    if (string.IsNullOrEmpty(source))
                        Debug.LogError(
                            "[TCP2 Shader Generator] Can't save Terrain Base Shader: source is null or empty!");
                    else
                    ...
                    if (string.IsNullOrEmpty(source))
                        Debug.LogError(
                            "[TCP2 Shader Generator] Can't save Terrain AddPass Shader: source is null or empty!");
                    else
                ...
                TCP2_ShaderGenerator.ShaderGeneratorTemplate template, Shader existingShader = null)
            {
                ...
                if (!keywords.ContainsKey("SHADER_TARGET"))
                {
                    ...
                    if (config.shaderTarget == 20) Utils.AddIfMissing(features, "FORCE_SM2");
                }
                ...
                string[] templateLines = template.textAsset.text.Split(new[] { "\r\n", "\n" }, StringSplitOptions.None);
                    ...
                    if (line.StartsWith("#"))
                    {
                        ...
                        if (line.StartsWith("#KEYWORDS"))
                        {
                            while (i < templateLines.Length)
                            {
                                ...
                                string error = ProcessKeywords(templateLines[i], config, ref features, ref flags,
                                    ref keywords, ref i, ref depth, ref stack, ref done);
            ...
            private static string ProcessKeywords(string line, TCP2_Config config, ref List<string> features,
                ref List<string> flags, ref Dictionary<string, string> keywords, ref int i, ref int depth,
                    ...
                    if ((depth >= 0 && stack[depth]) || depth < 0)
                    {
                        ...
                        switch (parts[0])
                        {
                            ...
                            case "disable_kw":
                                ...
                                break;
                            case "enable_flag":
                                Utils.AddIfMissing(flags, parts[1]);
                                break;
                            case "disable_flag":
                                Utils.RemoveIfExists(flags, parts[1]);
                                break;
            ...
            private static bool EvaluateExpression(int lineNumber, List<string> features, params string[] conditions)
            {
                ...
                string expression = "";
                for (int n = 2; n < conditions.Length; n++) expression += conditions[n];
            ...
            {
                string str = "";
                foreach (string s in array) str += s + separator;
            ...
            //Save .shader file
            private static Shader SaveShader(TCP2_Config config, Shader existingShader, string sourceCode,
                ...
                out Dictionary<string, string> Keywords, out string[] CustomData)
            {
                ...
                List<string> customDataList = new();
                Dictionary<string, string> keywordsDict = new();
            ...
            private static string GetIncludePrefix(TCP2_Config config)
            {
                ...
                string prefix = "";
                foreach (char c in config.Filename)
                    if (c == '/')
                        prefix += "../";
                return prefix;
            }
            ...
            private static string GetIncludeRelativePath(TCP2_Config config, Shader existingShader)
            {
                ...
                if (outputPath == OUTPUT_PATH)
                    return INCLUDE_REL_PATH;
                string tcp2includeFile = Utils.GetFileSafe(Application.dataPath, "TCP2_Include.cginc");
                if (tcp2includeFile != null)
                {
                    string absoluteTcp2IncludeDir = Path.GetDirectoryName(tcp2includeFile) + "/";
