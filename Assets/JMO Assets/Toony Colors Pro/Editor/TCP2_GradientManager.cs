    ...
    namespace Utilities
    {
		...
		ti.textureFormat = TextureImporterFormat.RGB24;
            ...
            public static bool SaveGradientTexture2D(Gradient[] gradients, int width, int height, string unityPath)
            {
                ...
                Texture2D texture = new(width, height, TextureFormat.ARGB32, true, true);
		...
		ti.textureFormat = TextureImporterFormat.RGB24;
                ...
                foreach (Gradient gradient in gradients)
                {
                    ...
                    for (int i = 0; i < gradient.colorKeys.Length; i++)
                        output += string.Format(CultureInfo.InvariantCulture, "{0},{1},",
                            ColorToHex(gradient.colorKeys[i].color), gradient.colorKeys[i].time);
                    output = output.TrimEnd(',');
                    output += ";";
                    for (int i = 0; i < gradient.alphaKeys.Length; i++)
                        output += string.Format(CultureInfo.InvariantCulture, "{0},{1},", gradient.alphaKeys[i].alpha,
            ...
            public static void LegacySetGradientFromUserData(string userData, Gradient gradient)
            {
                ...
                string[] ckData = keys[1].Split('#');
                GradientColorKey[] colorsKeys = new GradientColorKey[ckData.Length];
                for (int i = 0; i < ckData.Length; i++)
                {
                    string[] data = ckData[i].Split(',');
                    colorsKeys[i] = new GradientColorKey(HexToColor(data[0]),
                        float.Parse(data[1], CultureInfo.InvariantCulture));
                }

                string[] akData = keys[2].Split('#');
                GradientAlphaKey[] alphaKeys = new GradientAlphaKey[akData.Length];
                for (int i = 0; i < akData.Length; i++)
                {
                    string[] data = akData[i].Split(',');
                    alphaKeys[i] = new GradientAlphaKey(float.Parse(data[0], CultureInfo.InvariantCulture),
                        float.Parse(data[1], CultureInfo.InvariantCulture));
                }
            ...
            public static List<Gradient> GetGradientsFromUserData(string userData)
            {
                ...
                try
                {
                    ...
                    {
                        string[] split = userData.Split(new[] { "\n" }, StringSplitOptions.RemoveEmptyEntries);
                        foreach (string gradientString in split) list.Add(Deserialize(gradientString));
                    }
            ...
            {
                int height = 4;
                Texture2D ramp = new(width, height, TextureFormat.RGB24, true, true);
                Color[] colors = GetPixelsFromGradient(gradient, width, height);
            ...
            public static Color[] GetPixelsFromGradient(Gradient gradient, int width, int height, Color[] pixels)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int i = 0; i < height; i++) pixels[x + i * width] = col;
                }
            ...
            public static void SetPixelsFromGradients(Texture2D texture, Gradient[] gradients, int width, int height)
            {
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    Gradient g1 = gradients[rampIndex];
                    Gradient g2 = gradients[nextRampIndex];
                    ...
                    Color[] pixels = GetPixelsFromGradient(lerpGradient, width, 1, pixelsBuffer);
            ...
            private static Gradient Deserialize(string serializedGradient)
            {
                ...
                string[] split = serializedGradient.Substring("gradient:".Length).Split(';');
                List<GradientColorKey> colorKeys = new();
                List<GradientAlphaKey> alphaKeys = new();

                // parse color keys:
                string[] colorKeysStr = split[0].Split(',');
                for (int i = 0; i < colorKeysStr.Length; i += 2)
                    colorKeys.Add(new GradientColorKey(HexToColor(colorKeysStr[i]),
                        float.Parse(colorKeysStr[i + 1], CultureInfo.InvariantCulture)));

                // parse alpha keys:
                string[] alphaKeysStr = split[1].Split(',');
