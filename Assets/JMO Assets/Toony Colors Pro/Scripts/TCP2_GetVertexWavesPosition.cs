...
namespace ToonyColorsPro
{
    namespace Runtime
    {
        public class TCP2_GetVertexWavesPosition : MonoBehaviour
        {
            ...
            [Header("Water Shader Configuration")] [Space]
            public int sineCount = 1;

            [Space] public float WavesSpeed = 2.0f;

            public float WavesHeight = 0.1f;
            public float WavesFrequency = 1.0f;

            [Space] public bool useCustomTime;

            public bool customSineValues;

            // Shader Generator 2 values:
            [HideInInspector] public Vector4 sinOffsets1 = new(1.0f, 2.2f, 0.6f, 1.3f);
            [HideInInspector] public Vector4 phaseOffsets1 = new(1.0f, 1.3f, 2.2f, 0.4f);

            [HideInInspector] public Vector4 sinOffsets2 = new(0.6f, 1.3f, 3.1f, 2.4f);
            [HideInInspector] public Vector4 phaseOffsets2 = new(2.2f, 0.4f, 3.3f, 2.9f);

            [HideInInspector] public Vector4 sinOffsets3 = new(1.4f, 1.8f, 4.2f, 3.6f);
            [HideInInspector] public Vector4 phaseOffsets3 = new(0.2f, 2.6f, 0.7f, 3.1f);
            [HideInInspector] public Vector4 sinOffsets4 = new(1.1f, 2.8f, 1.7f, 4.3f);
            [HideInInspector] public Vector4 phaseOffsets4 = new(0.5f, 4.8f, 3.1f, 2.3f);
            ...
            private void LateUpdate()
            {
                ...
                {
                    transform.rotation =
                        Quaternion.FromToRotation(upAxis, GetNormalOnWater_SG2(time, transform.position));
                    transform.Rotate(postRotation, Space.Self);
                }
            }

            private Vector4 CalculateSinePosition(float v1, float v2, Vector4 sinOffsets, Vector4 phaseOffsets,
                ref float phase)
            {
                return new Vector4(
            ...
            private Vector4 CalculateSineNormal(float v1, float v2, Vector4 sinOffsets, Vector4 phaseOffsets,
                ref float phase)
            {
                return new Vector4(
            ...
            public Vector3 GetPositionOnWater_SG2(float time, Vector3 worldPosition)
            {
                ...
                switch (sineCount)
                {
                    ...
                    case 8:
                    {
                        ...
                        waveFactorX =
                            (sinWavesX.x + sinWavesX.y + sinWavesX.z + sinWavesX.w + sinWavesX2.x + sinWavesX2.y +
                             sinWavesX2.z + sinWavesX2.w) * height / 8.0f;
                        waveFactorZ =
                            (sinWavesZ.x + sinWavesZ.y + sinWavesZ.z + sinWavesZ.w + sinWavesZ2.x + sinWavesZ2.y +
                             sinWavesZ2.z + sinWavesZ2.w) * height / 8.0f;
                        break;
                    }
    ...
    {
#if UNITY_EDITOR
        [CustomEditor(typeof(TCP2_GetVertexWavesPosition))]
        [CanEditMultipleObjects]
        internal class TCP2_GetVertexWavesPosition_Editor : Editor
        {
            public override void OnInspectorGUI()
            {
                base.OnInspectorGUI();

                SerializedProperty customSineValues = serializedObject.FindProperty("customSineValues");
                if (customSineValues.boolValue)
                {
                    SerializedProperty sinOffsets1 = serializedObject.FindProperty("sinOffsets1");
                    SerializedProperty phaseOffsets1 = serializedObject.FindProperty("phaseOffsets1");
                    SerializedProperty sinOffsets2 = serializedObject.FindProperty("sinOffsets2");
                    SerializedProperty phaseOffsets2 = serializedObject.FindProperty("phaseOffsets2");
                    SerializedProperty sinOffsets3 = serializedObject.FindProperty("sinOffsets3");
                    SerializedProperty phaseOffsets3 = serializedObject.FindProperty("phaseOffsets3");
                    SerializedProperty sinOffsets4 = serializedObject.FindProperty("sinOffsets4");
                    SerializedProperty phaseOffsets4 = serializedObject.FindProperty("phaseOffsets4");
                    ...
                    EditorGUILayout.HelpBox(
                        "Change the phase and offset values if you have changed the default ones in the shader using the Shader Generator 2's Shader Properties system.",
                        MessageType.Info);
                }
                ...
                if (GUILayout.Button("Try to fetch values\nfrom water material", GUILayout.Height(30)))
                    ...
                    {
                        TCP2_GetVertexWavesPosition script = t as TCP2_GetVertexWavesPosition;
                        if (script != null && script.WaterPlane != null)
                        {
                            Renderer renderer = script.WaterPlane.GetComponent<Renderer>();
                            if (renderer != null && renderer.sharedMaterial != null)
                            {
                                Material mat = renderer.sharedMaterial;

                                if (mat.HasProperty("_WavesSpeed"))
                                    serializedObject.FindProperty("WavesSpeed").floatValue =
                                        mat.GetFloat("_WavesSpeed");

                                if (mat.HasProperty("_WavesHeight"))
                                    serializedObject.FindProperty("WavesHeight").floatValue =
                                        mat.GetFloat("_WavesHeight");

                                if (mat.HasProperty("_WavesFrequency"))
                                    serializedObject.FindProperty("WavesFrequency").floatValue =
                                        mat.GetFloat("_WavesFrequency");

                                SerializedProperty sineCountProperty = serializedObject.FindProperty("sineCount");
                                if (mat.shader != null)
                                {
            ...
            private bool FetchMaterialValue(Material material, string property, out float value)
            {
