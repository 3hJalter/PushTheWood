...
namespace UnityEditor
{
    internal class PolyboxShaderGUI : ShaderGUI
    {
        ...
        private readonly ColorPickerHDRConfig m_ColorPickerHDRConfig = new(0f, 99f, 1 / 99f, 3f);
        ...
        private MaterialProperty heightFogColor;
        private MaterialProperty heightFogEnd;

        private MaterialProperty heightFogStart;
        ...
        private void DoSpecularMetallicArea()
        {
            if (m_WorkflowMode == WorkflowMode.Specular)
            {
                if (specularMap.textureValue == null)
                    m_MaterialEditor.TexturePropertyTwoLines(Styles.specularMapText, specularMap, specularColor,
                        Styles.smoothnessText, smoothness);
                else
            ...
            else if (m_WorkflowMode == WorkflowMode.Metallic)
            {
                if (metallicMap.textureValue == null)
                    m_MaterialEditor.TexturePropertyTwoLines(Styles.metallicMapText, metallicMap, metallic,
                        Styles.smoothnessText, smoothness);
                else
        ...
        private static class Styles
        {
            ...
            public static readonly GUIContent
                detailAlbedoText = new("Detail Albedo x2", "Albedo (RGB) multiplied by 2");

            public static readonly GUIContent detailNormalMapText = new("Normal Map", "Normal Map");

            public static readonly GUIContent heightFogStart = new("Height Fog Start",
                "Start range for Height Fog measured in meters on Z axis");

            public static readonly GUIContent heightFogEnd = new("Height Fog End",
                "End range for Height for measured in meters on the Z axis");

            public static readonly GUIContent heightFogColor = new("Height Fog Color",
                "Height Fog Overide Color, if Alhpa is 0 then the Global Fog Color is used");
