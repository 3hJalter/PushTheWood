...
namespace CnControls
{
    ...
    public class CnInputManager
    {
        ...
        /// </summary>
        private readonly Dictionary<string, List<VirtualAxis>> _virtualAxisDictionary = new();
        ...
        /// </summary>
        private readonly Dictionary<string, List<VirtualButton>> _virtualButtonsDictionary = new();
        ...
        private static float GetAxis(string axisName, bool isRaw)
        {
            ...
            if (ButtonExists(axisName))
            {
                ...
                if (anyButtonIsPressed) return 1f;
            }
        ...
        public static bool GetButton(string buttonName)
        {
            ...
            if (ButtonExists(buttonName)) return GetAnyVirtualButton(Instance._virtualButtonsDictionary[buttonName]);
        ...
        public static bool GetButtonUp(string buttonName)
        {
            ...
            // If not, we check our virtual buttons
            if (ButtonExists(buttonName)) return GetAnyVirtualButtonUp(Instance._virtualButtonsDictionary[buttonName]);
        ...
        public static void UnregisterVirtualAxis(VirtualAxis virtualAxis)
        {
            ...
            if (Instance._virtualAxisDictionary.ContainsKey(virtualAxis.Name))
            {
                if (!Instance._virtualAxisDictionary[virtualAxis.Name].Remove(virtualAxis))
                    Debug.LogError("Requested axis " + virtualAxis.Name +
                                   " exists, but there's no such virtual axis that you're trying to unregister");
            }
            else
            {
        ...
        public static void UnregisterVirtualButton(VirtualButton virtualButton)
        {
            if (Instance._virtualButtonsDictionary.ContainsKey(virtualButton.Name))
            {
                if (!Instance._virtualButtonsDictionary[virtualButton.Name].Remove(virtualButton))
                    Debug.LogError(
                        "Requested button axis exists, but there's no such virtual button that you're trying to unregister");
            }
            else
            {
        ...
        private static float GetVirtualAxisValue(List<VirtualAxis> virtualAxisList, string axisName, bool isRaw)
        {
            ...
            var axisValue = isRaw ? Input.GetAxisRaw(axisName) : Input.GetAxis(axisName);
            if (!Mathf.Approximately(axisValue, 0f)) return axisValue;

            for (var i = 0; i < virtualAxisList.Count; i++)
            {
                var currentAxisValue = virtualAxisList[i].Value;
                if (!Mathf.Approximately(currentAxisValue, 0f)) return currentAxisValue;
            }
        ...
        {
            for (var i = 0; i < virtualButtons.Count; i++)
