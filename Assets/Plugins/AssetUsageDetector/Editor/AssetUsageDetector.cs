    ...
    public partial class AssetUsageDetector
    {
        ...
        public SearchResult Run(Parameters searchParameters)
        {
            ...
            try
            {
                ...
                {
                    foreach (CacheEntry cacheEntry in assetDependencyCache.Values)
                ...
                HashSet<string> folderContentsSet = new();
                ...
                HashSet<string> scenesToSearch = new();
                if (searchParameters.searchInScenesSubset != null && searchParameters.searchInScenesSubset.Length > 0)
                {
                    foreach (Object obj in searchParameters.searchInScenesSubset)
                    {
                        ...
                        if (obj.IsFolder())
                        {
                            string[] folderContents = AssetDatabase.FindAssets("t:SceneAsset",
                                new[] { AssetDatabase.GetAssetPath(obj) });
                ...
                if (isInPlayMode)
                {
                    ...
                    List<string> skippedScenes = new(scenesToSearch.Count);
                    ...
                    if (skippedScenes.Count > 0)
                    {
                        ...
                        sb.Append("Can't search unloaded scenes while in play mode, skipped ")
                            .Append(skippedScenes.Count).AppendLine(" scene(s):");
                ...
                {
                    currentSearchResultGroup =
                        new SearchResultGroup("Project Window (Assets)", SearchResultGroup.GroupType.Assets);
                ...
                {
                    currentSearchResultGroup =
                        new SearchResultGroup("Project Settings", SearchResultGroup.GroupType.ProjectSettings);
                ...
                if (scenesToSearch.Count > 0)
                {
                    ...
                    HashSet<string> excludedScenesPathsSet = new();
                    ...
                    {
                        if (EditorUtility.DisplayCancelableProgressBar("Please wait...",
                                "Searching scene: " + scenePath, (float)++searchProgress / searchTotalProgress))
                            throw new Exception("Search aborted");
                ...
                {
                    if (EditorUtility.DisplayCancelableProgressBar("Please wait...",
                            "Searching scene: DontDestroyOnLoad", 1f))
                        throw new Exception("Search aborted");

                    currentSearchResultGroup = new SearchResultGroup("DontDestroyOnLoad",
        ...
        {
            currentSearchResultGroup =
                new SearchResultGroup("Unused Objects", SearchResultGroup.GroupType.UnusedObjects, false);

            usedObjectsSet = new HashSet<Object>();
            HashSet<string>
                usedObjectPathsSet =
                    new(); // For assets: stores the filepaths, For scene objects: stores the topmost GameObject's instanceID
                ...
                for (int j = 0; j < searchResultGroup.NumberOfReferences; j++)
                {
                    ...
                    if (usedObjectsSet.Add(obj))
                    {
                        ...
                        if (!string.IsNullOrEmpty(assetPath))
                            ...
                            for (Transform parent = ((GameObject)obj).transform.parent;
                                 parent != null;
            ...
            Dictionary<string, ReferenceNode> unusedMainObjectNodes =
                new(objectsToSearchSet.Count - usedObjectsSet.Count);
            Dictionary<string, List<ReferenceNode>> unusedSubObjectNodes =
                new(objectsToSearchSet.Count - usedObjectsSet.Count);
            foreach (Object obj in objectsToSearchSet)
            {
                ...
                if (obj is GameObject)
                {
                    ...
                    if (searchedTopmostGameObject &&
                        !string.IsNullOrEmpty(
                            assetPath)) // Omit GameObject assets if their parent objects are already included in search
                        continue;
                }

                // Omit meshes of an imported model asset
                if (obj is Mesh && !string.IsNullOrEmpty(assetPath) &&
                ...
                if (string.IsNullOrEmpty(assetPath))
                {
                    ...
                    else // List child GameObject scene objects under their parent GameObject
                    {
                        ...
                        if (!unusedSubObjectNodes.TryGetValue(dictionaryKey, out unusedSubObjectNodesAtPath))
                            unusedSubObjectNodes[dictionaryKey] =
                                unusedSubObjectNodesAtPath = new List<ReferenceNode>(2);
        ...
        private void AddSearchedObjectToFilteredSets(Object obj, bool expandGameObjects)
        {
                ...
                openPrefabStage.IsPartOfPrefabContents((GameObject)obj))
            {
                GameObject prefabStageObjectSource =
                    ((GameObject)obj).FollowSymmetricHierarchy(openPrefabStage.prefabContentsRoot,
        ...
        private void SearchScene(string scenePath, List<SearchResultGroup> searchResult, Parameters searchParameters,
            SceneSetup[] initialSceneSetup)
        {
            ...
            bool canContainSceneObjectReference = scene.isLoaded && (!EditorSceneManager.preventCrossSceneReferences ||
                                                                     sceneObjectsToSearchScenesSet.Contains(scenePath));
            if (!canContainSceneObjectReference)
            {
                bool canContainAssetReference =
                    assetsToSearchSet.Count > 0 && (isInPlayMode || AssetHasAnyReference(scenePath));
            ...
            if (searchParameters.searchInSceneLightingSettings &&
                (!isInPlayMode || SceneManager.GetActiveScene() == scene))
            {
                if (!isInPlayMode && SceneManager.GetActiveScene() != scene)
        ...
        private ReferenceNode SearchObject(object obj)
        {
            ...
            if (unityObject != null)
            {
                ...
                if (assetPath != null &&
                    extensionToSearchFunction.TryGetValue(Utilities.GetFileExtension(assetPath), out searchFunction) &&
        ...
        private bool AssetHasAnyReference(string assetPath)
        {
            ...
            if (alwaysSearchedExtensionsSet.Count > 0 &&
        ...
        private bool AssetHasAnyReferenceInternal(string assetPath)
        {
            ...
            for (int i = 0; i < dependencies.Length; i++)
            {
                ...
                if (!Directory.Exists(
                        dependencies[i])) // Calling FileInfo.Length on a directory throws FileNotFoundException
                {
