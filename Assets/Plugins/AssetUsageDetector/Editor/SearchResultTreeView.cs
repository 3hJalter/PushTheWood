...
namespace AssetUsageDetectorNamespace
{
    ...
    public class SearchResultTreeView : TreeView
    {
                ...
                if (parent != null)
                {
                    ...
                    if (parent.node[linkIndex].descriptions.Count > 0)
                    {
                        ...
                        for (int i = 0; i < linkDescriptions.Count; i++)
                            sb.Append("  <color=#")
                                .Append(ColorUtility.ToHtmlStringRGBA(AssetUsageDetectorSettings
                                    .TooltipDescriptionTextColor)).Append(">").Append(linkDescriptions[i])
                                .Append("</color>\n");
                    }
        ...
        [Obsolete] private new string searchString { get; }
        [Obsolete] private new bool hasSearch { get; }
            ...
            bool usesExternalScrollView) : base(state)
        {
            ...
            highlightedSearchTextColor = "<color=#" +
                                         ColorUtility.ToHtmlStringRGBA(AssetUsageDetectorSettings
 ...
 typeof( TreeView ).GetField( "m_TreeView", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance ).GetValue( this );
        ...
        public void RefreshSearch(string prevSearchTerm)
        {
            ...
            if (!wasSearchTermEmpty || !isSearchTermEmpty)
            {
                ...
                else if (!wasSearchTermEmpty && state.preSearchExpandedIds != null &&
                         state.preSearchExpandedIds.Count > 0)
                {
                    List<int> expandedIds = state.preSearchExpandedIds;
                    HashSet<int> expandedIdsSet = new(expandedIds);
        ...
        {
            TreeViewItem root = new() { id = state.initialNodeId, depth = -1, displayName = "Root" };
            int id = state.initialNodeId + 1;

            idToNodeDataLookup.Clear();

            List<ReferenceNode> stack = new(8);
            ...
            if (hideDuplicateRows)
            {
                ...
                for (int i = references.Count - 1; i >= 0; i--)
                    ...
                    if (!isSearching || (state.searchMode != SearchMode.ReferencesOnly &&
                                         textComparer.IndexOf(references[i].Label, state.searchTerm,
            ...
            if (isTreeViewEmpty) // May happen if all items are hidden inside HideItems function or there are no matching search results. If we don't create a dummy child, Unity throws an exception
                root.AddChild(
                    new TreeViewItem(state.initialNodeId +
        ...
        private bool GenerateRowsRecursive(TreeViewItem parent, ReferenceNode referenceNode,
            ReferenceNodeData parentData, int siblingIndex, int depth, bool? itemForcedVisibility,
            List<ReferenceNode> stack, HashSet<ReferenceNode> processedNodes, ref int id)
        {
            ...
            if (hideReduntantPrefabVariantLinks && depth > 0)
            {
                ...
                if (unityObject && (unityObject is Component || unityObject is GameObject) && unityObject.IsAsset())
                {
                    ...
                    if (linkDescriptions.Count > 0)
                    {
                        ...
                        if (prefabObject &&
                            parentData.node.HasLinkToObjectWithDescriptions(prefabObject.GetInstanceID(),
                                linkDescriptions))
                            return false;
                    }
                }
            ...
            TreeViewItem item = new(id++, depth, "");
            ReferenceNodeData data = new(item, referenceNode, parentData, siblingIndex);
            ...
            else
            {
                ...
                else if (state.searchMode == SearchMode.All ||
                         depth == 0 == (state.searchMode == SearchMode.SearchedObjectsOnly))
                {
                    shouldShowItem = textComparer.IndexOf(referenceNode.Label, state.searchTerm, textCompareOptions) >=
                                     0;
                    ...
                    if (state.searchMode == SearchMode.SearchedObjectsOnly ||
                        (state.searchMode == SearchMode.All && shouldShowItem))
            ...
            if (referenceNode.NumberOfOutgoingLinks > 0 && !stack.ContainsFast(referenceNode))
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < referenceNode.NumberOfOutgoingLinks; i++)
                        shouldShowItem |= GenerateRowsRecursive(item, referenceNode[i].targetNode, data, i, depth + 1,
                            itemForcedVisibility, stack, processedNodes, ref id);
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            if (prevHoveredData != hoveredData)
            {
                ...
                if (hoveredData != null)
                {
                    ...
                    {
                        customTooltipShowTime = EditorApplication.timeSinceStartup +
                                                AssetUsageDetectorSettings.CustomTooltipDelay;
                        EditorApplication.update += ShowTooltipDelayed;
                    }
                }
                else
        ...
        protected override void RowGUI(RowGUIArgs args)
        {
                        ...
                        if (linkDescriptions.Count > 1)
                        {
                            ...
                            if (isSearching && state.searchMode != SearchMode.SearchedObjectsOnly)
                                for (int i = linkDescriptions.Count - 1; i > 0; i--)
                                    if (textComparer.IndexOf(linkDescriptions[i], state.searchTerm,
                                            textCompareOptions) >= 0)
                                    {
            ...
            if (ev.type == EventType.Repaint)
            {
                if (treeType != TreeType.UnusedObjects)
                {
                    if (args.item.depth == 0)
                    {
                        ...
                        if (AssetUsageDetectorSettings.RootRowsBorderColor.a > 0f)
                        {
                            ...
                            if (data.isLastLink || (args.item.hasChildren && IsExpanded(args.item.id)))
                            {
                                GUI.DrawTexture(
                                    new Rect(rect.x, rect.yMax - SEARCHED_OBJECTS_BORDER_THICKNESS, rect.width,
                                        SEARCHED_OBJECTS_BORDER_THICKNESS), EditorGUIUtility.whiteTexture,
                                    ScaleMode.StretchToFill, true, 0f);
                                GUI.DrawTexture(
                                    new Rect(rect.x, rect.y + 1, SEARCHED_OBJECTS_BORDER_THICKNESS,
                                        rect.height - 2f * SEARCHED_OBJECTS_BORDER_THICKNESS),
                                    EditorGUIUtility.whiteTexture, ScaleMode.StretchToFill, true, 0f);
                                GUI.DrawTexture(
                                    new Rect(rect.xMax - SEARCHED_OBJECTS_BORDER_THICKNESS, rect.y + 1,
                                        SEARCHED_OBJECTS_BORDER_THICKNESS,
                                        rect.height - 2f * SEARCHED_OBJECTS_BORDER_THICKNESS),
                                    EditorGUIUtility.whiteTexture, ScaleMode.StretchToFill, true, 0f);
                            }
                            else
                            {
                                GUI.DrawTexture(
                                    new Rect(rect.x, rect.y + 1, SEARCHED_OBJECTS_BORDER_THICKNESS, rect.height),
                                    EditorGUIUtility.whiteTexture, ScaleMode.StretchToFill, true, 0f);
                                GUI.DrawTexture(
                                    new Rect(rect.xMax - SEARCHED_OBJECTS_BORDER_THICKNESS, rect.y + 1,
                                        SEARCHED_OBJECTS_BORDER_THICKNESS, rect.height), EditorGUIUtility.whiteTexture,
                                    ScaleMode.StretchToFill, true, 0f);
                            }
                        }
                    ...
                    else
                    {
                        if (!args.selected)
                        {
                            ...
                            if (data.node.IsMainReference)
                                EditorGUI.DrawRect(
                                    new Rect(rect.x, rect.y, GetContentIndent(args.item) - 1f, rect.height),
                ...
                if (!isLMBDown && treeType != TreeType.UnusedObjects && !args.selected &&
                    selectedReferenceNodes.Contains(data.node))
                {
                    ...
                    GUI.DrawTexture(new Rect(GetContentIndent(args.item), rect.y, 125f, rect.height),
                        whiteGradientTexture, ScaleMode.StretchToFill, true, 0f);
                    GUI.color = guiColor;
                }
                ...
                if (AssetUsageDetectorSettings.ShowTreeLines && args.item.depth > 0)
                {
                    ...
                    Rect verticalLineRect =
                        new(
                            rect.x + GetContentIndent(args.item.parent) -
                            (foldoutWidth + TREE_VIEW_LINES_THICKNESS) * 0.5f - 2f, rect.y, TREE_VIEW_LINES_THICKNESS,
                            rect.height);
                    Rect horizontalLineRect = new(verticalLineRect.x,
                    ...
                    for (ReferenceNodeData parentData = data.parent;
                         parentData.parent != null;
                         parentData = parentData.parent)
                        if (!parentData.isLastLink)
                        {
                            shouldHighlightTreeLine =
                                selectedReferenceNodesHierarchyIndirectIds.Contains(parentData.item.id);
                            Rect _verticalLineRect =
                                new(verticalLineRect.x - depthIndentWidth * (args.item.depth - parentData.item.depth),
                                    verticalLineRect.y, verticalLineRect.width, verticalLineRect.height);
                            if (shouldHighlightTreeLine)
                            {
                                _verticalLineRect.x -=
                                    (HIGHLIGHTED_TREE_VIEW_LINES_THICKNESS - TREE_VIEW_LINES_THICKNESS) * 0.5f;
                            ...
                            GUI.DrawTexture(_verticalLineRect, EditorGUIUtility.whiteTexture, ScaleMode.StretchToFill,
                                true, 0f);
                        }

                    bool isInSelectedReferenceNodesHierarchy =
                        selectedReferenceNodesHierarchyIds.Contains(args.item.id);
                    if (isInSelectedReferenceNodesHierarchy)
                    {
                        horizontalLineRect.y -=
                            (HIGHLIGHTED_TREE_VIEW_LINES_THICKNESS - TREE_VIEW_LINES_THICKNESS) * 0.5f;
                    ...
                    GUI.color = guiColor * (isInSelectedReferenceNodesHierarchy
                        ? AssetUsageDetectorSettings.HighlightedTreeLinesColor
                        : AssetUsageDetectorSettings.TreeLinesColor);
                    GUI.DrawTexture(horizontalLineRect, EditorGUIUtility.whiteTexture, ScaleMode.StretchToFill, true,
                        0f);
                    ...
                    bool isInSelectedReferenceNodesIndirectHierarchy =
                        selectedReferenceNodesHierarchyIndirectIds.Contains(args.item.id);
                    if (isInSelectedReferenceNodesHierarchy || isInSelectedReferenceNodesIndirectHierarchy)
                    {
                        ...
                        {
                            if (!data.isLastLink)
                                verticalLineRect.height =
                                    (verticalLineRect.height + HIGHLIGHTED_TREE_VIEW_LINES_THICKNESS) * 0.5f;
                            else
                                verticalLineRect.height +=
                                    (HIGHLIGHTED_TREE_VIEW_LINES_THICKNESS - TREE_VIEW_LINES_THICKNESS) * 0.5f;
                        }
                        ...
                        GUI.DrawTexture(verticalLineRect, EditorGUIUtility.whiteTexture, ScaleMode.StretchToFill, true,
                            0f);
                    }
                ...
                foldoutLabelStyle.Draw(rect, sharedGUIContent, false, false, args.selected && args.focused,
                    args.selected);
            ...
            {
                if (hoveredData != data && AssetUsageDetectorSettings.ShowCustomTooltip &&
                    rect.Contains(ev.mousePosition))
                {
                    hoveredData = data;
                    hoveredDataRect = new Rect(GUIUtility.GUIToScreenPoint(rect.position),
                        new Vector2(EditorGUIUtility.currentViewWidth, 0f));
                }
            }
        }
        ...
        protected override void SelectionChanged(IList<int> selectedIds)
        {
            ...
            List<Object> selectedUnityObjects = new(selectedIds.Count);
                ...
                if (clickedPrefabRoot)
                {
                    ...
                    if (prefabAssetType == PrefabAssetType.Regular || prefabAssetType == PrefabAssetType.Variant)
                    {
                        ...
                        if (openPrefabStage == null || !openPrefabStage.stageHandle.IsValid() ||
                            assetPath != openPrefabStage.assetPath)
#else
        ...
        protected override void ContextClicked()
        {
            if (!isTreeViewEmpty && HasSelection() && HasFocus())
            {
                ...
                bool hasAnyDuplicateRows = false,
                    hasAnyRowWithOutgoingLinks = false,
                    hasAnyUnusedMixedCollapsedNode = false;
                for (int i = 0; i < selection.Count; i++)
                {
                    ...
                    if (!hasAnyUnusedMixedCollapsedNode &&
                        data.node.usedState == ReferenceNode.UsedState.MixedCollapsed)
                ...
                if (treeType == TreeType.UnusedObjects)
                {
                    if (hasAnyUnusedMixedCollapsedNode)
                    {
                        ...
                        contextMenu.AddItem(new GUIContent("Show Used Children"), false,
                            ShowChildrenOfSelectedUnusedObjects);
                    }
                }
                else
                {
                    ...
                    if (hasAnyDuplicateRows)
                        contextMenu.AddItem(new GUIContent("Select First Occurrence"), false,
                            SelectFirstOccurrencesOfDuplicateSelection);
                ...
                if (hasAnyRowWithOutgoingLinks)
                {
                    ...
                    contextMenu.AddItem(new GUIContent("Show Children In New Window"), false,
                        ShowChildrenOfSelectionInNewWindow);
                }
                ...
                if (Event.current != null && Event.current.type == EventType.ContextClick)
                    Event.current
                        .Use(); // It's safer to eat the event and if we don't, the context menu is sometimes displayed with a delay
            }
        }
        ...
        protected override void SetupDragAndDrop(SetupDragAndDropArgs args)
        {
            ...
            List<Object> draggedUnityObjects = new(draggedItemIds.Count);
        ...
        {
            List<int> expandedIds = new(rootItem.children.Count);
        ...
        {
            List<int> expandedIds = new(references.Count * 12);
            for (int i = 0; i < rootItem.children.Count; i++)
        ...
        private void ExpandAllSelectionOccurrences()
        {
            ...
            HashSet<ReferenceNode> selectedNodes = new();
            for (int i = selection.Count - 1; i >= 0; i--)
                selectedNodes.Add(GetDataFromId(selection[i]).node);

            List<int> expandedIds = new(GetExpanded());
            for (int i = 0; i < rootItem.children.Count; i++)
        ...
        private bool GetReferenceNodeOccurrenceIdsRecursive(TreeViewItem item, HashSet<ReferenceNode> referenceNodes,
            List<int> ids)
        {
            bool shouldExpand = false;
            if (item.hasChildren)
        ...
        private void HideItems(IList<int> ids)
        {
            if (ids.Count > 0)
            {
                List<ReferenceNode> hiddenNodes = new(ids.Count);
                List<ReferenceNode.Link> hiddenLinks = new(ids.Count);
                List<int> newExpandedItemIDs = new(32);
                List<int> newSelectedItemIDs = new(16);
                ...
                int id = state.initialNodeId + 1;
                for (int i = 0; i < rootItem.children.Count; i++)
                    CalculateNewItemIdsAfterHideRecursive(rootItem.children[i], hiddenNodes, hiddenLinks,
                        newExpandedItemIDs, newSelectedItemIDs, ref id);
        ...
        private void CalculateNewItemIdsAfterHideRecursive(TreeViewItem item, List<ReferenceNode> hiddenNodes,
            List<ReferenceNode.Link> hiddenLinks, List<int> newExpandedItemIDs, List<int> newSelectedItemIDs,
            ref int id)
        {
            ReferenceNodeData data = GetDataFromId(item.id);
            if (hiddenNodes.Contains(data.node) ||
                (data.parent != null && hiddenLinks.Contains(data.parent.node[data.linkIndex])))
                return;
            ...
            if (item.hasChildren)
                for (int i = 0; i < item.children.Count; i++)
                    CalculateNewItemIdsAfterHideRecursive(item.children[i], hiddenNodes, hiddenLinks,
                        newExpandedItemIDs, newSelectedItemIDs, ref id);
        }
        ...
        private void SelectFirstOccurrencesOfDuplicateSelection()
        {
            ...
            List<int> newSelection = new(selection.Count);
            for (int i = 0; i < rootItem.children.Count; i++)
            ...
            if (newSelection.Count > 0)
            {
                ...
                if (treeType != TreeType.IsolatedView)
                    ...
                    {
                        type = EventType.KeyDown, keyCode = KeyCode.F
                    }); // To actually frame the row when external scroll view is used
            }
        }

        private void FindFirstOccurrencesOfSelectionRecursive(TreeViewItem item, HashSet<ReferenceNode> selectedNodes,
            List<int> result)
        {
            ReferenceNodeData data = GetDataFromId(item.id);
            ...
            for (int i = selection.Count - 1; i >= 0; i--)
            {
                ...
                if (string.IsNullOrEmpty(assetPath))
                {
                    foreach (Object obj in usedObjectsSet)
                        ...
                        {
                            ReferenceNode childNode = new() { nodeObject = obj };
        ...
        private void ShowChildrenOfSelectionInNewWindow()
        {
            ...
            List<ReferenceNode> selectedNodes = new(selection.Count);
            for (int i = 0; i < selection.Count; i++)
            {
                ReferenceNodeData data = GetDataFromId(selection[i]);
            ...
            {
                SearchResultTreeView isolatedTreeView = new(new SearchResultTreeViewState(), selectedNodes,
                    TreeType.IsolatedView, null, hideDuplicateRows, hideReduntantPrefabVariantLinks, false);
                isolatedTreeView.ExpandMainReferences();

                SearchResultTreeViewIsolatedView.Show(
                    new Vector2(EditorWindow.focusedWindow.position.width,
                        Mathf.Max(isolatedTreeView.totalHeight, EditorGUIUtility.singleLineHeight * 5f) + 1f),
                    isolatedTreeView,
        ...
        public void GetRowStateWithId(int id, out bool isFirstRow, out bool isLastRow, out bool isExpanded,
            out bool canExpand)
        {
            if (isTreeViewEmpty)
            {
        ...
        private void HighlightSearchTermInString(StringBuilder sb, string str)
        {
            ...
            while ((searchOccurrenceIndex =
                       textComparer.IndexOf(str, state.searchTerm, searchOccurrenceIndex, textCompareOptions)) >= 0)
            {
            ...
            else
            {
                ...
                if (shouldRepositionSelf)
                {
                    ...
                    if (preferredHeight > 10f)
                    {
                        ...
                        position = Utilities.GetScreenFittedRect(new Rect(
                            GUIUtility.GUIToScreenPoint(Event.current.mousePosition) + new Vector2(size.x * -0.5f, 10f),
                            size));
