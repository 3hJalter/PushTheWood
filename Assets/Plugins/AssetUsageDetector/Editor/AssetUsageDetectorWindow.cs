...
#pragma warning restore 0649
        ...
        private readonly List<Object>
            searchInAssetsSubset = new() { null }; // If not empty, only these assets are searched for references
 ...
 (System.Collections.IList) preferencesWindowType.GetField( "m_Sections", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance ).GetValue( preferencesWindow );
        ...
        private static AssetUsageDetectorWindow GetWindow(WindowFilter filter)
        {
            ...
            AssetUsageDetectorWindow window = Array.Find(windows, w => w && !w.IsLocked);
            if (!window)
        ...
        private void OnGUI()
        {
            ...
            else if (currentPhase == Phase.Setup)
            {
				...
				EditorGUI.EndDisabledGroup();
				addressablesSupport =
 WordWrappingToggleLeft( "Addressables support (Experimental) (WARNING: 'Lazy scene search' will be disabled) (slower search)", addressablesSupport );
#endif
...
#if UNITY_2018_3_OR_NEWER
                hideReduntantPrefabVariantLinks = WordWrappingToggleLeft(
