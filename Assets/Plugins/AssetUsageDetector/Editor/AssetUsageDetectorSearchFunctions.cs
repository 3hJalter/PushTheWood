...
#pragma warning disable 0649 // The fields' values are assigned via JsonUtility
        ...
        private struct ShaderGraphReferences // Used by old Shader Graph serialization format
        {
            ...
            public class TextureHolder
            {
                ...
                {
                    string guid = ExtractGUIDFromString(!string.IsNullOrEmpty(m_SerializedTexture)
                        ? m_SerializedTexture
                        : m_SerializedCubemap);
...
#pragma warning restore 0649
        ...
        private readonly FieldInfoGetter fieldInfoGetter = (FieldInfoGetter)Delegate.CreateDelegate(
            typeof(FieldInfoGetter),
            typeof(Editor).Assembly.GetType("UnityEditor.ScriptAttributeUtility").GetMethod(
 ...
 (FieldInfoGetter) Delegate.CreateDelegate( typeof( FieldInfoGetter ), typeof( Editor ).Assembly.GetType( "UnityEditor.ScriptAttributeUtility" ).GetMethod( "GetFieldInfoFromProperty", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static ) );
        ...
        private readonly Func<Object> lightmapSettingsGetter =
            (Func<Object>)Delegate.CreateDelegate(typeof(Func<Object>),
                typeof(LightmapEditorSettings).GetMethod("GetLightmapSettings",
                    BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static));

        private readonly Func<Object> renderSettingsGetter = (Func<Object>)Delegate.CreateDelegate(typeof(Func<Object>),
            typeof(RenderSettings).GetMethod("GetRenderSettings",
                BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static));
#if UNITY_2021_2_OR_NEWER
        private readonly Func<Cubemap> defaultReflectionProbeGetter =
            (Func<Cubemap>)Delegate.CreateDelegate(typeof(Func<Cubemap>),
                typeof(RenderSettings).GetProperty("defaultReflection",
                    BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static).GetGetMethod(true));
#endif

#if ASSET_USAGE_ADDRESSABLES
		private readonly Func<SpriteAtlas, Sprite[]> spriteAtlasPackedSpritesGetter =
 (Func<SpriteAtlas, Sprite[]>) Delegate.CreateDelegate( typeof( Func<SpriteAtlas, Sprite[]> ), typeof( SpriteAtlasExtensions ).GetMethod( "GetPackedSprites", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static ) );
 ...
 Array.Find( Array.Find( AppDomain.CurrentDomain.GetAssemblies(), ( assembly ) => assembly.GetName().Name == "Unity.VisualEffectGraph.Editor" ).GetType( "UnityEditor.VFX.VFXSerializableObject" ).GetMethods( BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance ), ( methodInfo ) => methodInfo.Name == "Get" && !methodInfo.IsGenericMethod );
        ...
        private ReferenceNode SearchGameObject(object obj)
        {
            ...
            if (searchPrefabConnections)
            {
				...
				if( prefab )
#endif
                    if (objectsToSearchSet.Contains(prefab) &&
                        assetsToSearchRootPrefabs.ContainsFast(prefab as GameObject))
                    {
        ...
        private ReferenceNode SearchComponent(object obj)
        {
            ...
            if (searchMonoBehavioursForScript && component is MonoBehaviour)
            {
                ...
                if (objectsToSearchSet.Contains(script))
                {
                    ...
                    if (searchParameters.searchRefactoring != null)
                        searchParameters.searchRefactoring(new BehaviourUsageMatch(component.gameObject, script,
                            component));
                }
            }
            ...
            else if (component is Animator)
            {
                ...
                if (searchParameters.searchRefactoring != null && objectsToSearchSet.Contains(animatorController))
                    searchParameters.searchRefactoring(new AnimationSystemMatch(component, animatorController,
                        newValue => ((Animator)component).runtimeAnimatorController =
                    ...
                    foreach (PlayableBinding binding in playableAsset.outputs)
                    {
                        ...
                        if (searchParameters.searchRefactoring != null && objectsToSearchSet.Contains(bindingValue))
                            searchParameters.searchRefactoring(new AnimationSystemMatch(component, bindingValue,
                                newValue => ((PlayableDirector)component).SetGenericBinding(binding.sourceObject,
                                    newValue)));
                    }
            }
#endif
                ...
                if (isInPlayMode && !AssetDatabase.Contains(component))
                {
                    ...
                    try
                    {
...
#endif
                        {
                            ...
                            if (searchParameters.searchRefactoring != null && objectsToSearchSet.Contains(plane))
                                searchParameters.searchRefactoring(new OtherSearchMatch(collisionModule, plane,
                                    component, newValue => collisionModule.SetPlane(i, (Transform)newValue)));
                        }
                    }
                    catch
                    {
                    ...
                    try
                    {
...
#endif
                        {
                            ...
                            if (searchParameters.searchRefactoring != null && objectsToSearchSet.Contains(collider))
                                searchParameters.searchRefactoring(new OtherSearchMatch(triggerModule, collider,
                                    component, newValue => triggerModule.SetCollider(i, (Component)newValue)));
                        }
                    }
                    catch
                    {
                    ...
                    try
                    {
                        ...
                        for (int i = 0, j = textureSheetAnimationModule.spriteCount; i < j; i++)
                        {
                            ...
                            if (searchParameters.searchRefactoring != null && objectsToSearchSet.Contains(sprite))
                                searchParameters.searchRefactoring(new OtherSearchMatch(textureSheetAnimationModule,
                                    sprite, component,
                    ...
                    try
                    {
                        ...
                        for (int i = 0, j = subEmittersModule.subEmittersCount; i < j; i++)
                        {
                            ...
                            referenceNode.AddLinkTo(SearchObject(subEmitterSystem),
                                "Sub Emitters Module: ParticleSystem");

                            if (searchParameters.searchRefactoring != null &&
                                objectsToSearchSet.Contains(subEmitterSystem))
                                searchParameters.searchRefactoring(new OtherSearchMatch(subEmittersModule,
                                    subEmitterSystem, component,
        ...
        private ReferenceNode SearchMaterial(object obj)
        {
            ...
            for (int i = referenceNode.NumberOfOutgoingLinks - 1; i >= 0; i--)
            {
                        ...
                        if (texturePropertyEndIndex > texturePropertyPrefixIndex)
                        {
                            ...
                            if (int.TryParse(
                                    linkDescriptions[j].Substring(texturePropertyPrefixIndex,
                                        texturePropertyEndIndex - texturePropertyPrefixIndex),
                                    out texturePropertyIndex))
                            {
                                if (materialSO == null)
                                    materialSO = new SerializedObject(material);

                                string propertyName = materialSO
                                    .FindProperty("m_SavedProperties.m_TexEnvs.Array.data[" + texturePropertyIndex +
                                                  "].first").stringValue;
                ...
                if (linkDescriptions.Count ==
                    0) // All shader properties were unused and we weren't searching for unused material properties
            ...
            if (searchTextureReferences && isInPlayMode && !AssetDatabase.Contains(material))
            {
                ...
                for (int i = 0; i < shaderPropertyCount; i++)
                    if (ShaderUtil.GetPropertyType(shader, i) == ShaderUtil.ShaderPropertyType.TexEnv)
                    {
                        ...
                        {
                            referenceNode.AddLinkTo(GetReferenceNode(assignedTexture),
                ...
                if (shaderImporter != null)
                {
                    ...
                    for (int i = 0; i < shaderPropertyCount; i++)
                        if (ShaderUtil.GetPropertyType(shader, i) == ShaderUtil.ShaderPropertyType.TexEnv)
                        {
                            ...
                            {
                                referenceNode.AddLinkTo(GetReferenceNode(defaultTexture),
                                    "Default Texture: " + propertyName);

                                if (searchParameters.searchRefactoring != null)
                                    searchParameters.searchRefactoring(
                                        new AssetImporterDefaultValueMatch(shaderImporter, defaultTexture, propertyName,
                                            null));
                            }
                        }
                }
        ...
        private ReferenceNode SearchMonoScript(object obj)
        {
            ...
            for (int i = monoScriptsToSearch.Count - 1; i >= 0; i--)
                if (monoScriptsToSearchTypes[i] != scriptType &&
                    monoScriptsToSearchTypes[i].IsAssignableFrom(scriptType))
            ...
            if (scriptImporter != null)
            {
                ...
                for (int i = 0; i < variables.Length; i++)
                    if (variables[i].isSerializable && !variables[i].IsProperty)
                    {
                        ...
                        {
                            referenceNode.AddLinkTo(GetReferenceNode(defaultValue),
                                "Default variable value: " + variables[i].Name);

                            if (searchParameters.searchRefactoring != null)
                                searchParameters.searchRefactoring(new AssetImporterDefaultValueMatch(scriptImporter,
                                    defaultValue, variables[i].Name, variables));
                        }
                    }
            }
        ...
        private ReferenceNode SearchAnimatorController(object obj)
        {
            ...
            if (controller is AnimatorController)
            {
                ...
                for (int i = 0; i < layers.Length; i++)
                {
                    if (objectsToSearchSet.Contains(layers[i].avatarMask))
                    {
                        ...
                        if (searchParameters.searchRefactoring != null)
                            searchParameters.searchRefactoring(new AnimationSystemMatch(layers[i], layers[i].avatarMask,
                                controller, newValue => layers[i].avatarMask = (AvatarMask)newValue));
                    }
            ...
            else
            {
                ...
                {
                    RuntimeAnimatorController parentController =
                        ((AnimatorOverrideController)controller).runtimeAnimatorController;
                    ...
                    if (searchParameters.searchRefactoring != null)
                    {
                        List<KeyValuePair<AnimationClip, AnimationClip>> overrideClips =
                            new(((AnimatorOverrideController)controller).overridesCount);
                        ...
                        for (int i = overrideClips.Count - 1; i >= 0; i--)
                            if (objectsToSearchSet.Contains(overrideClips[i].Value))
                                searchParameters.searchRefactoring(new AnimationSystemMatch(controller,
                                    overrideClips[i].Value, newValue =>
                                    {
                                        overrideClips[i] =
                                            new KeyValuePair<AnimationClip, AnimationClip>(overrideClips[i].Key,
                                                (AnimationClip)newValue);
            ...
            if (searchMonoBehavioursForScript)
            {
                ...
                for (int i = 0; i < behaviours.Length; i++)
                {
                    ...
                    if (objectsToSearchSet.Contains(script))
                    {
                        ...
                        if (searchParameters.searchRefactoring != null)
                            searchParameters.searchRefactoring(new BehaviourUsageMatch(animatorStateMachine, script,
                                behaviours[i]));
                    }
                }
            }
            ...
            if (searchMonoBehavioursForScript)
            {
                ...
                for (int i = 0; i < behaviours.Length; i++)
                {
                    ...
                    if (objectsToSearchSet.Contains(script))
                    {
                        ...
                        if (searchParameters.searchRefactoring != null)
                            searchParameters.searchRefactoring(new BehaviourUsageMatch(animatorState, script,
                                behaviours[i]));
                    }
                }
            }
        ...
        private ReferenceNode SearchAnimationClip(object obj)
        {
            ...
            for (int i = 0; i < objectCurves.Length; i++)
            {
                ...
                for (int j = 0; j < keyframes.Length; j++)
                {
                    ...
                    if (searchParameters.searchRefactoring != null && objectsToSearchSet.Contains(keyframes[j].value))
                        searchParameters.searchRefactoring(new AnimationSystemMatch(clip, keyframes[j].value,
                            newValue =>
                            {
            ...
            {
                referenceNode.AddLinkTo(SearchObject(events[i].objectReferenceParameter),
                    "AnimationEvent: " + events[i].time);

                if (searchParameters.searchRefactoring != null &&
                    objectsToSearchSet.Contains(events[i].objectReferenceParameter))
                    searchParameters.searchRefactoring(new AnimationSystemMatch(clip,
        ...
        private ReferenceNode SearchSpriteAtlas(object obj)
        {
            ...
            SerializedObject spriteAtlasSO = new(spriteAtlas);
            if (spriteAtlas.isVariant)
            {
                ...
                if (objectsToSearchSet.Contains(masterAtlas))
                {
                    ...
                    if (searchParameters.searchRefactoring != null)
                        searchParameters.searchRefactoring(new SerializedPropertyMatch(spriteAtlas, masterAtlas,
                            masterAtlasProperty));
                }
            }
            ...
            if (packables != null)
            {
                for (int i = 0, length = packables.arraySize; i < length; i++)
                {
                    ...
                    if (searchParameters.searchRefactoring != null && objectsToSearchSet.Contains(packedSprite))
                        searchParameters.searchRefactoring(new SerializedPropertyMatch(spriteAtlas, packedSprite,
                            packedSpriteProperty));
                }
            }
#if UNITY_2018_2_OR_NEWER
            else
        ...
        private void SearchSpriteAtlas(ReferenceNode referenceNode, Object packedAsset)
        {
            ...
            if (packedAsset is Texture)
            {
                ...
                {
                    Object[] textureSubAssets =
                        AssetDatabase.LoadAllAssetRepresentationsAtPath(AssetDatabase.GetAssetPath(packedAsset));
            ...
            else if (packedAsset.IsFolder())
            {
                ...
                string[] texturesInFolder =
                    AssetDatabase.FindAssets("t:Texture2D", new[] { AssetDatabase.GetAssetPath(packedAsset) });
        ...
        private ReferenceNode SearchAssemblyDefinitionFile(object obj)
        {
            ...
            AssemblyDefinitionReferences assemblyDefinitionFile =
                JsonUtility.FromJson<AssemblyDefinitionReferences>(((TextAsset)obj).text);
            ReferenceNode referenceNode = PopReferenceNode(obj);
            ...
            if (assemblyDefinitionFile.references != null)
                for (int i = 0; i < assemblyDefinitionFile.references.Count; i++)
                {
                    ...
                    string assemblyPath =
                        CompilationPipeline.GetAssemblyDefinitionFilePathFromAssemblyReference(assemblyDefinitionFile
                            .references[i]);
#else
					string assemblyPath =
 CompilationPipeline.GetAssemblyDefinitionFilePathFromAssemblyName( assemblyDefinitionFile.references[i] );
                    ...
                    if (!string.IsNullOrEmpty(assemblyPath))
                    {
                        ...
                        if (assemblyDefinitionFilesToSearch.TryGetValue(assemblyPath,
                                out searchedAssemblyDefinitionFile))
                            referenceNode.AddLinkTo(GetReferenceNode(searchedAssemblyDefinitionFile),
                                "Referenced Assembly");
                    }
                }
            ...
            else
            {
                ...
                if (shaderGraph.m_SerializedProperties != null)
                    for (int i = shaderGraph.m_SerializedProperties.Count - 1; i >= 0; i--)
                    {
                        ...
                        ShaderGraphReferences.PropertyData propertyData =
                            JsonUtility.FromJson<ShaderGraphReferences.PropertyData>(propertyJSON);
                        ...
                        if (objectsToSearchSet.Contains(texture))
                            referenceNode.AddLinkTo(GetReferenceNode(texture),
                                "Default Texture: " + propertyData.GetName());
                    }
                ...
                if (shaderGraph.m_SerializableNodes != null)
                    for (int i = shaderGraph.m_SerializableNodes.Count - 1; i >= 0; i--)
                    {
                        ...
                        ShaderGraphReferences.NodeData nodeData =
                            JsonUtility.FromJson<ShaderGraphReferences.NodeData>(nodeJSON);
                        if (!string.IsNullOrEmpty(nodeData.m_FunctionSource))
                        {
                            ...
                            if (!string.IsNullOrEmpty(customFunctionPath) &&
                                assetsToSearchPathsSet.Contains(customFunctionPath))
                            {
                                Object customFunction = AssetDatabase.LoadMainAssetAtPath(customFunctionPath);
                                if (objectsToSearchSet.Contains(customFunction))
                                    referenceNode.AddLinkTo(GetReferenceNode(customFunction),
                                        "Used in node: " + nodeData.m_Name);
                            }
                        }
                        ...
                        for (int j = nodeData.m_SerializableSlots.Count - 1; j >= 0; j--)
                        {
                            ...
                            string texturePath = JsonUtility.FromJson<ShaderGraphReferences.NodeSlotData>(nodeSlotJSON)
                                .GetTexturePath();
                            if (string.IsNullOrEmpty(texturePath) || !assetsToSearchPathsSet.Contains(texturePath))
                                continue;
        ...
        private void SearchAnimatedObjects(ReferenceNode referenceNode)
        {
            ...
            for (int i = 0; i < clips.Length; i++)
            {
                ...
                if (!animationClipUniqueBindings.TryGetValue(clip, out uniqueBindings))
                {
                    ...
                    List<EditorCurveBinding> _uniqueBindings = new(2);
        ...
        private void SearchShaderSourceCodeForCGIncludes(ReferenceNode referenceNode)
        {
            ...
            IterateOverValuesInString(File.ReadAllText(shaderPath), new[] { "#include ", "#include_with_pragmas " },
                '"', include =>
                {
                    ...
                    if (!isIncludePotentialReference)
                    {
                        ...
                        int trimStartLength =
                            ...
                            1; // Convert absolute path to a Project-relative path
        ...
        private void SearchVariablesWithSerializedObject(ReferenceNode referenceNode,
            bool forceUseSerializedObject = false)
        {
            if (!isInPlayMode || referenceNode.nodeObject.IsAsset() || forceUseSerializedObject)
            {
                ...
                SerializedObject so = new((Object)referenceNode.nodeObject);
                ...
                if (iterator.Next(true))
                {
                    ...
                    do
                    {
                        ...
                        bool isVisible = iteratingVisible &&
                                         SerializedProperty.EqualContents(iterator, iteratorVisible);
                        if (isVisible)
                        {
                            iteratingVisible =
                                iteratorVisible.NextVisible(iteratorVisible.propertyType ==
                                                            SerializedPropertyType.Generic);
                        }
                        else
                        {
                            Type propFieldType;
                            isVisible = iterator.type == "Array" ||
                                        fieldInfoGetter(iterator, out propFieldType) != null;
                        }
                        ...
                        switch (iterator.propertyType)
                        {
                            ...
                            case SerializedPropertyType.Generic:
								...
								{
									object vfxSerializableObjectValue =
 vfxSerializableObjectValueGetter.Invoke( vfxSerializableObject, null );
                        ...
                        if (searchResult != null && searchResult != referenceNode)
                        {
                            ...
                            if (!propertyPath.EndsWithFast("m_RD.texture"))
                            {
                                ...
                                if (searchParameters.searchRefactoring != null &&
                                    objectsToSearchSet.Contains(propertyValue))
                                    searchParameters.searchRefactoring(
                                        new SerializedPropertyMatch((Object)referenceNode.nodeObject, propertyValue,
                                            iterator));
                            }
                        }
        ...
        private void SearchVariablesWithReflection(ReferenceNode referenceNode)
        {
            ...
            for (int i = 0; i < variables.Length; i++)
            {
                ...
                try
                {
                    ...
                    if (!(variableValue is ICollection))
                    {
                        ...
                        if (searchResult != null && searchResult != referenceNode)
                        {
                            ...
                            if (searchParameters.searchRefactoring != null &&
                                objectsToSearchSet.Contains(variableValue as Object))
                                searchParameters.searchRefactoring(new ReflectionMatch(referenceNode.nodeObject,
                                    (Object)variableValue, variables[i].variable));
                        }
                    }
                    ...
                    if (variableValue is IEnumerable && !(variableValue is Transform))
                    {
                        ...
                        foreach (object element in (IEnumerable)variableValue)
                        {
                            ...
                            {
                                referenceNode.AddLinkTo(searchResult,
                                    string.Concat(variables[i].IsProperty ? "Property: " : "Variable: ",
                                        variables[i].Name, "[", index + "]"));

                                if (searchParameters.searchRefactoring != null &&
                                    objectsToSearchSet.Contains(element as Object))
                                {
                        ...
                        if (foundReferences != null)
                            for (int j = foundReferences.Count - 1; j >= 0; j--)
                                searchParameters.searchRefactoring(new ReflectionMatch(referenceNode.nodeObject,
                                    foundReferences[j], variableValue));
                    }
                }
                ...
                catch (Exception e)
                {
                    ...
                    sb.Append("Skipped searching ").Append(referenceNode.nodeObject.GetType().FullName).Append(".")
                        .Append(variables[i].Name).AppendLine(" because it threw exception:").Append(e).AppendLine();
                ...
                while (currType != typeof(object))
                {
                    ...
                    for (int i = 0; i < fields.Length; i++)
                    {
                        ...
                        if (typeof(Graphic).IsAssignableFrom(currType) &&
                            (fieldName == "m_RectTransform" || fieldName == "m_CanvasRenderer" ||
                             fieldName == "m_Canvas"))
                            continue;
            ...
            if (propertyModifiers != (BindingFlags.Instance | BindingFlags.DeclaredOnly))
            {
                ...
                while (currType != typeof(object))
                {
                    ...
                    for (int i = 0; i < properties.Length; i++)
                    {
                        ...
                        if (typeof(MeshFilter).IsAssignableFrom(currType) && propertyName == "mesh") continue;
                        // Same as above
                        if ((propertyName == "material" || propertyName == "materials") &&
                            (typeof(Renderer).IsAssignableFrom(currType) ||
...
#pragma warning restore 0618
                        ...
                        if ((propertyName == "color" || propertyName == "mainTexture") &&
                            typeof(Material).IsAssignableFrom(currType)) continue;
                        ...
                        if (typeof(Animator).IsAssignableFrom(currType) && propertyName == "parameters") continue;
                        ...
                        if (propertyName == "spriteAnimator" && currType.Name == "TMP_Text") continue;
                        ...
                        if (propertyName == "meshFilter" &&
                            (currType.Name == "TextMeshPro" || currType.Name == "TMP_SubMesh")) continue;
                        ...
                        if (propertyName == "users" && typeof(TerrainData).IsAssignableFrom(currType)) continue;
            ...
            for (int i = 0; i < path.Length; i++)
            {
                ...
                else
                {
                    ...
                    int arrayElementIndex =
                        int.Parse(pathElement.Substring(arrayStartIndex + 1, arrayEndIndex - arrayStartIndex - 1));
		...
		private Object GetAddressablesAssetReferenceValue( AssetReference assetReference )
		{
			...
			if( !string.IsNullOrEmpty( subObjectName ) )
			{
				...
				{
					Type subObjectType =
 (Type) assetReferenceSubObjectTypeGetter.GetValue( assetReference, null ) ?? typeof( Object );
					Object[] subAssets =
 AssetDatabase.LoadAllAssetRepresentationsAtPath( AssetDatabase.GetAssetPath( result ) );
        ...
        private void IterateOverValuesInString(string str, string[] valuePrefixes, char valueWrapperChar,
