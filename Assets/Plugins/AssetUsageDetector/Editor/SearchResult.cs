...
namespace AssetUsageDetectorNamespace
{
    ...
    public class SearchResult : IEnumerable<SearchResultGroup>, ISerializationCallbackReceiver
    {
        ...
        public SearchResult(bool success, List<SearchResultGroup> result, HashSet<Object> usedObjects,
            SceneSetup[] initialSceneSetup, AssetUsageDetector searchHandler,
        ...
        void ISerializationCallbackReceiver.OnBeforeSerialize()
        {
            ...
            Dictionary<ReferenceNode, int> nodeToIndex = new(result.Count * 16);
        ...
        void ISerializationCallbackReceiver.OnAfterDeserialize()
        {
            ...
            List<ReferenceNode> allNodes = new(serializedNodes.Count);
            ...
            {
                result.Add(new SearchResultGroup(serializedGroups[i].title, serializedGroups[i].type,
                    serializedGroups[i].isExpanded, serializedGroups[i].pendingSearch));
        ...
        public void RefreshSearchResultGroup(SearchResultGroup searchResultGroup, bool noAssetDatabaseChanges)
        {
            ...
            SceneSearchMode searchInScenes = SearchParameters.searchInScenes;
            Object[] searchInScenesSubset = SearchParameters.searchInScenesSubset;
            bool searchInAssetsFolder = SearchParameters.searchInAssetsFolder;
            Object[] searchInAssetsSubset = SearchParameters.searchInAssetsSubset;
            bool searchInProjectSettings = SearchParameters.searchInProjectSettings;
            bool lazySceneSearch = SearchParameters.lazySceneSearch;
            bool calculateUnusedObjects = SearchParameters.calculateUnusedObjects;
            bool _noAssetDatabaseChanges = SearchParameters.noAssetDatabaseChanges;
            ...
            try
            {
                ...
                else if (searchResultGroup.Type == SearchResultGroup.GroupType.DontDestroyOnLoad)
                {
                    SearchParameters.searchInScenes =
                        (SceneSearchMode)1024; // A unique value to search only the DontDestroyOnLoad scene
                    SearchParameters.searchInScenesSubset = null;
                    SearchParameters.searchInAssetsFolder = false;
                    SearchParameters.searchInAssetsSubset = null;
                    SearchParameters.searchInProjectSettings = false;
                ...
                SearchResult searchResult = searchHandler.Run(SearchParameters);
                ...
                {
                    SearchResultGroup newSearchResultGroup =
                        searchResult.result.Find(group => group.Title == searchResultGroup.Title);
                    ...
                    SearchResultGroup unusedObjectsSearchResultGroup =
                        result.Find(group => group.Type == SearchResultGroup.GroupType.UnusedObjects);
                    if (unusedObjectsSearchResultGroup != null)
                    {
                        SearchResultGroup newUnusedObjectsSearchResultGroup =
                        ...
                        else
                        {
                            ...
                            HashSet<Object> newUnusedObjectsSet = new();
        ...
        public bool RestoreInitialSceneSetup()
        {
            ...
            for (int i = 0; i < initialSceneSetup.Length; i++)
            {
                Scene scene = SceneManager.GetSceneByPath(initialSceneSetup[i].path);
                if (!scene.isLoaded)
                ...
                if (assetPath == null)
                {
                    ...
                    if (other.assetPath == null)
                        return other.transform
                            ? Utilities.CompareHierarchySiblingIndices(transform, other.transform)
                            : -1;
                    return -1;
                }

                if (other.assetPath == null) return other.transform ? 1 : -1;
    ...
    public class SearchResultGroup : IEnumerable<ReferenceNode>
    {
        ...
        public SearchResultGroup(string title, GroupType type, bool isExpanded = true, bool pendingSearch = false)
        {
            ...
            ScenePath = type != GroupType.Scene ? null :
                title.StartsWith("<b>") ? title.Substring(3, title.Length - 7) : title;
            Type = type;
        ...
        {
            List<ReferenceNode> _references = new(references);
            references.Clear();

            // Reverse the links of the search results graph so that the root ReferenceNodes are the searched objects
            Dictionary<ReferenceNode, ReferenceNode> reverseGraphNodes = new(references.Count * 16);
            ...
            HashSet<ReferenceNode> visitedNodes = new();
            ...
            HashSet<ReferenceNode> rootGameObjectNodes = new();
            for (int i = references.Count - 1; i >= 0; i--)
        ...
        public float DrawOnGUI(SearchResult searchResult, EditorWindow window, float scrollPosition,
            bool noAssetDatabaseChanges)
        {
            ...
            if (GUI.Button(headerRect, Title, Utilities.BoxGUIStyle))
            {
                ...
                else
                {
                    ...
                    if (searchResult != null)
                        contextMenu.AddItem(new GUIContent("Hide"), false,
                            () => searchResult.RemoveSearchResultGroup(this));
                    ...
                    if (references.Count > 0 && treeView != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(treeViewState.searchTerm) && treeViewState.searchMode ==
                            SearchResultTreeView.SearchMode.ReferencesOnly)
                        ...
                        if (searchResult != null && searchResult.NumberOfGroups > 1 &&
                            !string.IsNullOrEmpty(treeViewState.searchTerm))
                        {
                            if (contextMenu.GetItemCount() > 0)
                                contextMenu.AddSeparator("");
                            ...
                            contextMenu.AddItem(new GUIContent("Apply Search to All Results"), false, () =>
                            {
                                for (int i = 0; i < searchResult.NumberOfGroups; i++)
                                {
                                    ...
                                    SearchResultTreeView.SearchMode previousSearchMode =
                                        searchResult[i].treeViewState.searchMode;
                                    ...
                                    if (treeViewState.searchTerm != previousSearchTerm ||
                                        treeViewState.searchMode != previousSearchMode)
                    ...
                    if (Type == GroupType.Scene && !EditorApplication.isPlaying && loadedSceneCount > 1)
                    {
                        ...
                        if (scene.isLoaded)
                        {
                            ...
                            {
                                if (!scene.isDirty ||
                                    EditorSceneManager.SaveModifiedScenesIfUserWantsTo(new Scene[1] { scene }))
            ...
            if (IsExpanded)
            {
                if (PendingSearch)
                {
                    GUILayout.Box("Lazy Search: this scene potentially has some references, hit Refresh to find them",
                        Utilities.BoxGUIStyle);
                }
                else if (references.Count == 0)
                {
                    GUILayout.Box(
                        Type == GroupType.UnusedObjects ? "No unused objects left..." : "No references found...",
                        Utilities.BoxGUIStyle);
                }
                else
                {
                    ...
                    if (treeView == null)
                    {
                        ...
                        bool shouldUpdateInitialTreeViewNodeId =
                            treeViewState.initialNodeId == 0 && searchResult != null;
                        ...
                        treeView = new SearchResultTreeView(treeViewState, references,
                            Type == GroupType.UnusedObjects
                                ...
                                : SearchResultTreeView.TreeType.Normal,
                            searchResult != null ? searchResult.UsedObjects : null,
                            searchResult != null && searchResult.SearchParameters.hideDuplicateRows,
                            searchResult != null && searchResult.SearchParameters.hideReduntantPrefabVariantLinks,
                            true);
                    ...
                    treeViewState.searchTerm = treeViewSearchField.OnToolbarGUI(
                        new Rect(searchFieldRect.x, searchFieldRect.y, searchFieldRect.width - 100f,
                            searchFieldRect.height), treeViewState.searchTerm) ?? "";
                    treeViewState.searchMode = (SearchResultTreeView.SearchMode)EditorGUI.EnumPopup(
                        new Rect(searchFieldRect.xMax - 100f, searchFieldRect.y, 100f, searchFieldRect.height),
                    ...
                    else if ((ev.type == EventType.ValidateCommand || ev.type == EventType.ExecuteCommand) &&
                             ev.commandName == "Find" && treeView.HasFocus())
                    {
                        if (ev.type == EventType.ExecuteCommand)
                        {
                            ...
                            scrollPosition = FrameRectInScrollView(scrollPosition,
                                new Vector2(searchFieldRect.y - 2f,
                                    searchFieldRect.yMax + EditorGUIUtility.singleLineHeight + 2f),
                    ...
                    bool isFirstRowSelected = false,
                        isLastRowSelected = false,
                        isSelectedRowExpanded = false,
                        canExpandSelectedRow = false;
                    if (treeViewKeyboardNavigation && treeView.HasFocus() && treeView.HasSelection())
                        treeView.GetRowStateWithId(treeViewState.lastClickedID, out isFirstRowSelected,
                            out isLastRowSelected, out isSelectedRowExpanded, out canExpandSelectedRow);
                    ...
                    if (ev.type == EventType.Repaint)
                    {
						...
						treeView.visibleRowBottom =
 treeView.visibleRowTop + (int) Mathf.Ceil( heightInPixels / treeView.rowHeight );
#endif
                    }
                        ...
                        if (treeView.GetRowRectWithId(treeViewState.lastClickedID, out targetTreeViewRowRect))
                        {
                            ...
                            if (!ev.control && !ev.command && !ev.shift)
                            {
                                if (isFirstRowSelected && (pressedKeyboardNavigationKey == KeyCode.UpArrow ||
                                                           ...
                                                           (pressedKeyboardNavigationKey == KeyCode.LeftArrow &&
                                                            !isSelectedRowExpanded)))
                                {
                                    ...
                                    for (int i = searchResultGroupIndex - 1; i >= 0; i--)
                                            ...
                                            searchResult[i].references.Count > 0)
                                        {
                                            ...
                                            targetTreeViewRowRect =
                                                searchResult[i].treeView.SelectLastRowAndReturnRect();
                                            ...
                                            break;
                                                               ...
                                                               (pressedKeyboardNavigationKey == KeyCode.RightArrow &&
                                                                !canExpandSelectedRow)))
                                {
                                    int searchResultGroupIndex = searchResult.IndexOf(this);
                                    for (int i = searchResultGroupIndex + 1; i < searchResult.NumberOfGroups; i++)
                                            ...
                                            searchResult[i].references.Count > 0)
                                        {
                                            ...
                                            targetTreeViewRect = searchResult[i].lastTreeViewRect;
                                            targetTreeViewRowRect =
                                                searchResult[i].treeView.SelectFirstRowAndReturnRect();
        ...
        {
            SearchResult.SerializableResultGroup serializedResultGroup = new()
            {
                title = Title,
                type = Type,
                isExpanded = IsExpanded,
            ...
            if (references != null)
            {
                ...
                for (int i = 0; i < references.Count; i++)
                    serializedResultGroup.initialSerializedNodes.Add(references[i]
                        .SerializeRecursively(nodeToIndex, serializedNodes));
            }
        ...
        internal void Deserialize(SearchResult.SerializableResultGroup serializedResultGroup,
    ...
    public class ReferenceNode
    {
        ...
        public void InitializeRecursively()
        {
            ...
            if (unityObject != null)
            {
                ...
                if (AssetUsageDetectorSettings.ShowRootAssetName && unityObject.IsAsset() &&
        ...
        public ReferenceNode CreateReverseGraphRecursively(SearchResultGroup searchResultGroup,
            List<ReferenceNode> reverseGraphRoots, Dictionary<ReferenceNode, ReferenceNode> reverseGraphNodes,
            HashSet<Object> objectsToSearchSet)
        {
            ...
            if (!reverseGraphNodes.TryGetValue(this, out result))
            {
                ...
                //else // When 'else' is uncommented, 'Don't search "Find referenced of" themselves for references" option simply does nothing. I am not entirely sure if commenting it out will have any side effects, so fingers crossed?
                {
                    for (int i = 0; i < links.Count; i++)
                    {
                        ReferenceNode linkedNode = links[i].targetNode.CreateReverseGraphRecursively(searchResultGroup,
                            reverseGraphRoots, reverseGraphNodes, objectsToSearchSet);
                    ...
                    switch (searchResultGroup.Type)
                    {
                        ...
                        case SearchResultGroup.GroupType.ProjectSettings:
                            result.IsMainReference = obj.IsAsset() &&
                                                     AssetDatabase.GetAssetPath(obj).StartsWith("ProjectSettings/");
                            break;
        ...
        internal int SerializeRecursively(Dictionary<ReferenceNode, int> nodeToIndex,
            List<SearchResult.SerializableNode> serializedNodes)
        {
            int index;
            ...
            SearchResult.SerializableNode serializedNode = new()
            {
                label = Label,
            ...
            if (links.Count > 0)
            {
                ...
                serializedNode.linkDescriptions =
                    new List<SearchResult.SerializableNode.SerializableLinkDescriptions>(links.Count);
        ...
        public class Link
        {
            ...
            public bool
                isWeakLink; // Weak links can be omitted from search results if this ReferenceNode isn't referenced by any other node
